...
const DogCard = ({ imageAlt, imageUrl, isFavorite, onClick, isSmall }: DogCardProps) => {
  const handleClick = () => onClick?.(imageUrl);

  const imageSize = isSmall ? 176 : 240;

  return (
    <div className={`relative bg-white border border-gray-200 rounded-lg ${isSmall ? 'w-44 h-44' : 'w-60 h-60'}`} onClick={handleClick}>
      <Image
